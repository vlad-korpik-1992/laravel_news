        ...
        <div class="row">
            <div class="col-lg-9 mx-auto">
                <section class="related-posts">
                    <h2 class="section-title mb-4" data-aos="fade-up">Смотрите также</h2>
                    <div class="row">
                ...
                <section class="comment-list pb-5">
                    <h2 class="secttion-title pb-3" date-aos="fade-up">Комментарии ({{ $post->comments->count() }})</h2>
                    @foreach ($post->comments as $comment)
                        <div class="comment-text pb-2">
                            <span class="username">
                                <div>{{ $comment->user->name }}</div>
                                <span class="text-nuted float-right">{{ $comment->dateAsCarbon->diffForHumans() }}</span>
                            </span>
                            {{ $comment->message }}
                        </div>
                    @endforeach
                        ...
                        <form action="{{ route('post.comment.store', $post->id) }}" method="post">
                            ...
                            <div class="row">
                                <div class="form-group col-12" data-aos="fade-up">
                                    ...
                                    <textarea name="message" id="message" class="form-control" placeholder="Оставить комментарий" rows="10"></textarea>
                                </div>
                            </div>
                            <div class="row">
